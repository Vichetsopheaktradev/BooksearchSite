...
function App() {
  ...
  const handleSaveClick = () => {
    const newBook = {
      id: savedBooks.length + 1,
      title: `Book ${savedBooks.length + 1}`,
    };
    setSavedBooks([...savedBooks, newBook]);
    setShowSaveList(true); // Show the save list popup
  };

  // Close SaveList popup
  const closeSaveList = () => {
    ...
    <div>
      ...
      {showAbout && (
        <Popup
          book={{
            title: "About Us",
            description: `We are a passionate group of students from the Royal University of Phnom Penh (RUPP),
      ...
      {showSaveList && (
        <SaveList savedBooks={savedBooks} onClose={closeSaveList} />
      )}
    </div>
  );
}
